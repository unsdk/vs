...
Refresh the searchable plugin index from the configured registry source:
...
- `vs update` with no arguments refreshes the registry index.
- `vs update <plugin>` refreshes one locally added plugin.
- `vs update --all` refreshes all locally added plugins.
