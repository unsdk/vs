...
`vs` is a Rust 2024 workspace (`Cargo.toml`) with crates under `crates/`. Keep boundaries tight: the thin `vs` crate owns the `vs` binary and delegates to `vs_cli::run()`; `vs-cli` owns argument parsing, commands, and TUI/output as a library; `vs-core` owns orchestration and use cases; leaf crates such as `vs-config`, `vs-registry`, `vs-installer`, and `vs-shell` provide focused services. Follow the existing file discipline: one CLI command per file in `crates/vs-cli/src/command/`, one service per file in `crates/vs-core/src/service/`. Shared test helpers live in `crates/vs-test-support`, fixtures in `fixtures/`, and longer-form docs in `docs/`.

## Build, Test, and Development Commands
- `cargo build -p vs`: build the default CLI at `target/debug/vs`.
- `cargo run -p vs -- --help`: run the CLI locally.
- `cargo build -p vs --no-default-features --features lua`: test a backend-specific build. Swap `lua` for `wasi` or use `"lua,wasi"`.
...
## Testing Guidelines
Prefer focused unit tests close to the code with `#[cfg(test)]`, and add integration coverage in `crates/vs/tests/` for end-to-end CLI flows. Reuse `vs-test-support` and `fixtures/` instead of creating ad hoc test data. Match the existing naming style: `resolve_home_should_prefer_vs_home`, `cli_should_migrate_from_a_legacy_home`.
